...
const EditPoDetails = ({ navigation, route }) => {
  ...
  const handleUpdatePurchaseOrder = async () => {
    ...
    const updatedPurchaseOrder = {
      ...
      due_date: 0,
      remarks: "test",
      date: formData?.billDate || null,
      warehouse_id: formData?.warehouse?.id ?? null,
      warehouse_name: formData?.warehouse?.label ?? null,
      update_purchase_line_ids: purchaseOrderLines.map((item) => ({
        purchase_line_ids: item.purchase_line_ids,
        product: item.product.product_id || item.product_id,
      ...
      create_purchase_line_ids: purchaseOrderLines.map((item) => ({
