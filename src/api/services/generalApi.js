...
export const processInvoiceWithPaymentOdoo = async ({ partnerId, products = [], journalId, invoiceDate = null, reference = '', paymentAmount = null } = {}) => {
  try {
    ...
    const postPaymentResponse = await fetch(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
      method: 'POST',
    ...
    const paymentStatusResponse = await fetch(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        jsonrpc: '2.0',
    ...
    if (!paymentDetails.reconciled) {
      ...
      const reconcileResponse = await fetch(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          jsonrpc: '2.0',
    ...
    const invoiceStatusResponse = await fetch(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        jsonrpc: '2.0',
...
export const updatePosOrderOdoo = async (orderId, values) => {
  try {
    ...
    const response = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
      jsonrpc: '2.0',
      method: 'call',
      params: {
        model: 'pos.order',
...
export const fetchPOSRegisters = async ({ limit = null, offset = 0 } = {}) => {
  try {
    ...
    const response = await axios.post(
      `${getOdooBaseUrl()}/web/dataset/call_kw`,
      {
        jsonrpc: "2.0",
        method: "call",
        params: {
...
import { getOdooBaseUrl, getOdooDb } from '@api/config/odooConfig';
...
export const fetchProductsOdoo = async () => {
  try {
    ...
    if (response.data.error) {
      console.log("Odoo JSON-RPC error (product filling):", response.data.error);
      throw new Error("Odoo JSON-RPC error");
    }

    const results = response.data.result || [];

    // Collect product ids to fetch images in batch
    const productIds = Array.from(new Set(results
      .map(r => Array.isArray(r.product_id) ? r.product_id[0] : r.product_id)
      .filter(Boolean)
    ));
    ...
    if (productIds.length > 0) {
      try {
        const prodResp = await axios.post(
          `${getOdooBaseUrl()}/web/dataset/call_kw`,
          {
            ...
            params: {
              ...
              kwargs: { fields: ['id', 'image_128', 'image_1920'], limit: productIds.length },
            },
          },
          { headers: { 'Content-Type': 'application/json' } }
        );

        const prodResults = (prodResp.data && prodResp.data.result) || [];
        const imgMap = {};
        ...
        prodResults.forEach(p => {
          ...
          imgMap[p.id] = hasBase64 ? `data:image/png;base64,${p.image_128}` : `${baseUrl}/web/image?model=product.product&id=${p.id}&field=image_128`;
        });

        // Attach image_url to each vending filling record
        results.forEach(r => {
          const pid = Array.isArray(r.product_id) ? r.product_id[0] : r.product_id;
          if (pid && imgMap[pid]) {
            r.image_url = imgMap[pid];
          } else {
            r.image_url = null;
          }
        });
      } catch (imgErr) {
        console.warn('Failed to fetch product images from Odoo:', imgErr);
        // fallback: leave image_url null
        results.forEach(r => { r.image_url = null; });
      }
    } else {
      // No products referenced
      results.forEach(r => { r.image_url = null; });
    }
...
export const fetchUserApiToken = async (uid) => {
  if (!uid) return { api_token: null, api_keys: [] };
  try {
    // 1) Use search_read to safely fetch the user record and fields
    const readPayload = {
      jsonrpc: '2.0',
      method: 'call',
      params: {
        ...
        method: 'search_read',
        args: [[['id', '=', uid]]],
        kwargs: { fields: ['id', 'api_token', 'api_key_ids', 'name'], limit: 1 },
      },
      id: new Date().getTime(),
    };

    const userResp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, readPayload, { headers: { 'Content-Type': 'application/json' } });
    if (userResp.data && userResp.data.error) {
      console.error('Odoo JSON-RPC error (res.users api_token):', userResp.data.error);
      return { api_token: null, api_keys: [], error: userResp.data.error };
    }

    const userRecord = Array.isArray(userResp.data.result) ? userResp.data.result[0] : null;
    const apiToken = userRecord?.api_token || null;
    if (apiToken) return { api_token: apiToken, api_keys: [] };

    // 2) Fallback: fetch ir.api.key records for the user
    const keyPayload = {
      jsonrpc: '2.0',
      method: 'call',
      params: {
        ...
        method: 'search_read',
        args: [[['user_id', '=', uid]]],
        kwargs: { fields: ['id', 'name', 'key', 'create_uid'], limit: 50 },
      },
      id: new Date().getTime(),
    };

    const keyResp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, keyPayload, { headers: { 'Content-Type': 'application/json' } });
    if (keyResp.data && keyResp.data.error) {
      console.error('Odoo JSON-RPC error (ir.api.key search_read):', keyResp.data.error);
      return { api_token: null, api_keys: [], error: keyResp.data.error };
    }

    const keys = Array.isArray(keyResp.data.result) ? keyResp.data.result : (keyResp.data.result || []);
    const apiKeys = keys.map(k => ({ id: k.id, name: k.name, key: k.key }));
    return { api_token: null, api_keys: apiKeys };
  } catch (error) {
    console.error('fetchUserApiToken error:', error);
    return { api_token: null, api_keys: [], error };
  }
};
...
export const fetchCategoriesOdoo = async ({ offset = 0, limit = 50, searchText = "" } = {}) => {
  try {
    ...
    const response = await axios.post(
      `${getOdooBaseUrl()}/web/dataset/call_kw`, // Odoo API URL
      {
        jsonrpc: "2.0",
        method: "call",
        params: {
...
export const createAccountPaymentOdoo = async ({ partnerId, journalId, amount, invoiceId = null } = {}) => {
  try {
    ...
    const response = await fetch(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (result.result) {
      ...
      await fetch(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          jsonrpc: '2.0',
...
export const createInvoiceOdoo = async ({ partnerId, products = [], journalId = null, invoiceDate = null, reference = '' } = {}) => {
  try {
    ...
    const createResp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
      jsonrpc: '2.0',
      method: 'call',
      params: {
        model: 'account.move',
    ...
    if (createdId) {
      try {
        const moveResp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
          jsonrpc: '2.0',
          method: 'call',
          params: {
            model: 'account.move',
      ...
      try {
        const linesResp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
          jsonrpc: '2.0',
          method: 'call',
          params: {
    ...
    if (createdId) {
      try {
        ...
        const postResp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
          jsonrpc: '2.0',
          method: 'call',
          params: {
            model: 'account.move',
    ...
    if (createdId) {
      try {
        const statusResp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
          jsonrpc: '2.0',
          method: 'call',
          params: {
            model: 'account.move',
...
export const linkInvoiceToPosOrderOdoo = async ({ orderId, invoiceId, setState = true, state = null } = {}) => {
  try {
    ...
    const resp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
      jsonrpc: '2.0',
      method: 'call',
      params: {
        model: 'pos.order',
    ...
    try {
      const verify = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
        jsonrpc: '2.0',
        method: 'call',
        params: {
          model: 'pos.order',
...
export const confirmSaleOrderOdoo = async (orderId, options = {}) => {
  try {
    ...
    const response = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, rpcPayload, { headers: { 'Content-Type': 'application/json' } });
    if (response.data && response.data.error) {
  ...
  try {
    ...
    for (const payment of paymentRecords) {
      ...
      if (!finalPaymentMethodId) {
        ...
        const pmResp = await axios.post(`${getOdooBaseUrl()}/web/dataset/call_kw`, {
          jsonrpc: '2.0',
          method: 'call',
          params: {
